...
var zhMessages = map[string]string{
	...
	"usage":                                 "LD-gpt-check\n\n用法：\n  ld-gpt-check setup [--lang zh-CN|en]\n  ld-gpt-check login [--api-base-url URL] [--lang zh-CN|en]\n  ld-gpt-check run [--backend auto|codex|api] [-m MODEL] [--api-format openai-chat|openai-responses|anthropic-messages] [--model-api-base-url URL] [--model-api-key KEY] [--suite candy_21] [--question-file FILE] [--question-url URL] [--no-remote-questions] [-r low|medium|high|xhigh] [-n TESTS] [--timeout 30m] [--upload] [--json]\n  ld-gpt-check run --list-suites [--question-file FILE] [--question-url URL] [--no-remote-questions]\n  ld-gpt-check update [--yes|--check-only]\n  ld-gpt-check config\n  ld-gpt-check whoami\n  ld-gpt-check logout",
