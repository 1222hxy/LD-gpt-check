...
- `POST /api/v1/admin/bridges/identify`：管理员提交 base URL，服务端抓取网页 title/icon，用于补全中转站配置。
- `POST /api/v1/admin/bridge-suggestions/status`：管理员把候选中转站恢复为 pending 或归档为 rejected。
...
`device_token` 是 CLI 设备授权密钥，只展示给本地 CLI。`access_token` 是兼容旧客户端的同值别名，新客户端应保存 `device_token` 到本地授权文件的 `device_authorization.secret`。
...
| `slow_down` | 增加轮询间隔后继续 |
| `expired` | 停止登录并提示重新开始 |
| `authorized` | 保存 `device_token` 为本地设备授权 secret；不要保存用户 ID |
...
4. `pending` 时继续等待。
5. `slow_down` 时增加间隔。
6. `authorized` 时保存 `device_token` 为本地设备授权 secret，不保存用户 ID。
7. `expired` 时提示重新登录。
