...
- `authorized`：授权完成，响应包含 `device_token`；旧 `access_token` 字段只作为兼容别名。
- `expired`：授权码过期或已消费。
...
管理员后台可对候选执行网页信息读取：服务端抓取候选站点首页的 title 和 icon URL，用网页 title、host 和用户提交名称辅助补全表单。候选可以被采纳为正式中转站，也可以归档为 `rejected`。
