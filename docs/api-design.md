...
| 中转站映射页面 | `GET /admin/bridges` | 浏览器 HTML 路由 |
...
管理后台 `/admin`、题目管理页面 `/admin/questions` 和中转站映射页面 `/admin/bridges` 也使用该 Web session。用户必须已通过 Linux.do 登录，且 Linux.do UID 命中 Worker 的管理员列表 `ADMIN_LINUXDO_IDS`。当前默认管理员 UID 是 `29368`。账号页 `/account` 只在当前用户是管理员时显示管理后台入口。
...
Linux.do Connect 实测 userinfo 还会返回 `api_key`。公开 API 和数据库都不得保存或暴露 `api_key`、provider access token、provider user id、session hash 或 token hash。
...
- `attempts[]` 可包含 `cached_input_tokens`、`total_tokens`、`event_count`、`event_types`、`codex_thread_id` 等诊断摘要。
- `submission` 必须包含 `codex_provider_base_url`。服务端规范化该 URL 后落库，并计算 `codex_channel`：`official`、`bridge` 或 `unknown_bridge`。
- `submission` 可包含 `codex_model_source`、`codex_model_provider`、`codex_provider_host`、sandbox 和禁用功能摘要。
- 不保存原始 Codex JSONL event、完整模型回答、完整 prompt、环境变量、本地路径或 Codex 数据库。

## 中转站映射

管理员可通过 `/admin/bridges` 配置全局中转站映射。一个中转站可以拥有多个 base URL，同一个 base URL 不能归属多个中转站。用户可在 `/account` 提交自己的中转站候选；上传遇到未知 provider base URL 时，服务端也会自动写入候选表并累计出现次数。上传时：

- `https://api.openai.com` 和 `https://api.openai.com/v1` 自动标记为 `official`。
- 命中 `bridge_base_urls.base_url` 时标记为 `bridge`，并记录 `codex_bridge_id`。
- 如果没有精确命中，但同 host 只有一个已启用中转站，也自动标记为该 `bridge`。
- 其它 HTTPS base URL 标记为 `unknown_bridge`。

管理员后台可对候选执行自动识别：服务端抓取候选站点首页的 title 和 icon URL；如配置 `BRIDGE_AI_BASE_URL`、`BRIDGE_AI_MODEL` 与 `BRIDGE_AI_API_KEY`，还会调用 OpenAI-compatible chat completions 生成中转站名称、slug、置信度和简短理由。未配置 AI 时使用网页 title、host 和用户提交名称兜底。

## 删除数据

当前用户可以删除自己的 benchmark 数据：

- `DELETE /api/v1/submissions/{id}`：删除单条 submission 及关联 attempts/question results。
- `DELETE /api/v1/submissions`：删除当前用户全部 benchmark submissions。
- 删除范围只包含测试数据，不删除 `users`、`web_sessions`、`access_tokens` 或 OAuth/device session。
- 单条删除对不存在或不属于当前用户的 id 返回 `{ ok: true, deleted: 0 }`，避免暴露资源归属。
...
- `attempts[].event_types`：短字符串数组。
- `codex_invocation`：安全净化后的调用摘要，不能包含 prompt、本地路径、token 或 secret。
